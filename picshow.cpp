    ...
    , ui(new Ui::PicShow),_selected_path(""),_b_btnvisible(false)
{
    ...
    opacity_pre->setOpacity(0);//设置透明度的，透明范围是0-1，
...
{
    if(_selected_path != ""){
        const auto &width = ui->gridLayout_2->geometry().width();
        const auto &height = ui->gridLayout_2->geometry().height();
...
void PicShow::SlotUpdatePic(const QString &_path)
{
    ...
    if(_selected_path!=""){
        ...
        const auto &height = ui->gridLayout_2->geometry().height();
        _pix_map.load(_selected_path);
        auto width1 = width-20;
        auto height1 = height-20;
        qDebug()<<"width:"<<width1<<",height:"<<height1;
        _pix_map = _pix_map.scaled(width1,height1,Qt::KeepAspectRatio);
        ui->label->setPixmap(_pix_map);
    }
}
